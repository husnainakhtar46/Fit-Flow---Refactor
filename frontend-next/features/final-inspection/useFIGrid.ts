...
export function useFIGrid({
  ...
  const columnKeys = [
    ...
    'tol',
    'std',
    ...Array.from({ length: sampleCount }, (_, i) => `sample_${i + 1}`),
  ];

  const getCellId = (rowIndex: number, columnKey: string) => `${rowIndex}:${columnKey}`;
  const isSelected = (rowIndex: number, columnKey: string) =>
    selectedCells.has(getCellId(rowIndex, columnKey));
  ...
  const handleMeasurementPaste =
    (rowIndex: number, startColumn: string) =>
